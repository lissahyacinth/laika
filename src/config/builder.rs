...
use crate::matcher::builder::MatchPatternBuilder;
use crate::matcher::{EventMatchPattern, EventMatcher, EventType};
...
pub struct EventProcessorYamlSpec {
    ...
    pub events: HashMap<String, MatchOptionsBuilder>,
    ...
    fn try_from(value: EventProcessorYamlSpec) -> LaikaResult<Self> {
        ...
        let event_matcher = EventMatcher::new(
            value
                .events
                .into_iter()
                .map(|(event_type, event_match_options)| {
                    event_match_options
                        .try_into()
                        .map(|match_option| (match_option, event_type))
                })
                .collect::<LaikaResult<Vec<(EventMatchPattern, EventType)>>>()?,
        );
