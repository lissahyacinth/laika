...
impl EventProcessor {
    ...
    pub fn parse_event(&self, raw_event: RawEvent) -> LaikaResult<Vec<Event>> {
    ...
    fn emit_action(
        ...
        Ok(EventAction::Emit(EmitAction::new(
            ...
            serde_json::to_value(action_config.emit_template.clone().render(&output)?).map_err(|e| LaikaError::TemplateError(e.to_string()))?,
        )))
    }
