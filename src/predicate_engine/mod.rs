...
mod tests {
    ...
    fn test_basic_predicate() -> LaikaResult<()> {
        ...
        let predicate = engine.store_predicate("(trigger, ctx) => trigger.event.active === true ? trigger : null");
    ...
    fn test_string_predicate() -> LaikaResult<()> {
        let mut engine = JsonPredicateEngine::new();
        let predicate = engine.store_predicate("(trigger, ctx) => trigger.event.type === 'test' ? trigger: null");
        let events: Vec<Event> = vec![
